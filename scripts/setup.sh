...
if test -f "${index}.1.bt2l"; then
    echo "Index exists"
else
    $bowtie2build $genome $index
fi
s=86
for i in  {434..535};
do
    ...
    res_un=data/unmapped/sar${i}-2019_S${s}_L001.unmapped.fastq
    ...
    $bowtie2 --very-sensitive -x $index -1 $sample_filter_1 -2 $sample_filter_2 -S $res --un-conc $res_un
